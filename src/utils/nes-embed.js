...
function onAnimationFrame() {
  canvas.requestAnimationFrame(onAnimationFrame);
...
function nes_init(_callback, canvas_res) {
  ...
  console.log('sw_sh', SCREEN_WIDTH, SCREEN_HEIGHT);
  console.log('w_h', width, height);

  canvas = canvas_res[0].node;
  canvas_ctx = canvas.getContext('2d');
  const dpr = Taro.getSystemInfoSync().pixelRatio;
  SCREEN_WIDTH = width * dpr;
  SCREEN_HEIGHT = height * dpr;
...
// ======================================================================
export function loadData(_canvas_id, rom_data) {
...
export function loadUrl(_canvas_id, rom_data_url) {
  Taro.request({
